...
import { getStrategyById, updateStrategyById } from 'apiSdk/strategies';
...
function StrategyEditPage() {
  ...
  const handleSubmit = async (values: StrategyInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateStrategyById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<StrategyInterface>({
    initialValues: data,
