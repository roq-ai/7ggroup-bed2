...
import { deleteStrategyById } from 'apiSdk/strategies';
...
function BusinessViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface>(
    ...
    () =>
      getBusinessById(id, {
        relations: ['user', 'feedback', 'strategy'],
      }),
  );
  ...
  const strategyHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/feedbacks/create?business_id=${data?.id}`}>
              ...
              {hasAccess('strategy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/strategies/create?business_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.strategy?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/strategies/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.type}</Td>
                            <Td>{record.content}</Td>
                            <Td>
                              {hasAccess('strategy', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/strategies/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('strategy', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    strategyHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
